...
test.describe('Manage tokens', () => {
  ...
  test('that token can be removed from asset list and added back', async ({ homePage }) => {
    ...
    const brc20InManageTokensList = homePage.manageTokensAssetsList.getByTestId(MockedTokensSelectors.Brc20TokenTestId);

    // src20 token
    const src20InAssetList = homePage.assetList.getByTestId(MockedTokensSelectors.Src20TokenTestId);
    const src20InManageTokensList = homePage.manageTokensAssetsList.getByTestId(MockedTokensSelectors.Src20TokenTestId);

    // rune token
    const runeInAssetList = homePage.assetList.getByTestId(MockedTokensSelectors.RuneTokenTestId);
    const runeInManageTokensList = homePage.manageTokensAssetsList.getByTestId(MockedTokensSelectors.RuneTokenTestId);
    ...
    const stx20InManageTokensList = homePage.manageTokensAssetsList.getByTestId(MockedTokensSelectors.Stx20TokenTestId);
