...
test.describe('send stx: tests on testnet', () => {
  ...
  test('that we show high fee warning in case of high custom fee', async ({ sendPage, page }) => {
    ...
    await page.getByTestId(SharedComponentsSelectors.CustomFeeFieldInput).fill(highFeeAmount.toString());
  ...
  test('that empty memo on preview matches default empty value', async ({ sendPage }) => {
    ...
    const confirmationMemo = await sendPage.memoRow.getByTestId(SharedComponentsSelectors.InfoCardRowValue).innerText();

    test.expect(confirmationMemo).toEqual(emptyMemoPreviewValue);
  });

  test('that asset value, recipient, memo and fees on preview match input', async ({ sendPage }) => {
    ...
    const fees = await sendPage.page.getByTestId(SharedComponentsSelectors.FeeToBePaidLabel).innerText();
    await sendPage.previewSendTxButton.click();
    ...
    const confirmationFees = await sendPage.feesRow.getByTestId(SharedComponentsSelectors.InfoCardRowValue).innerText();
  ...
  test.describe('send form preview', () => {
    test('send form preview: that it shows preview of tx details to be confirmed', async ({ sendPage }) => {
      await sendPage.amountInput.fill('0.000001');
      await sendPage.recipientInput.fill(TEST_TESTNET_ACCOUNT_2_STX_ADDRESS);
      await sendPage.previewSendTxButton.click();
      const details = await sendPage.confirmationDetails.allInnerTexts();
...
test.describe('send stx: tests on mainnet', () => {
  ...
  test.describe('send form input fields', () => {
    test('that recipient address matches bns name', async ({ sendPage }) => {
      ...
      const bnsResolvedAddress = await sendPage.page.getByText(TEST_BNS_RESOLVED_ADDRESS).innerText();

      test.expect(bnsResolvedAddress).toBeTruthy();
    });

    test('that fee row defaults to middle fee estimation', async ({ sendPage }) => {
      const feeToBePaid = await sendPage.page.getByTestId(SharedComponentsSelectors.FeeToBePaidLabel).innerText();
