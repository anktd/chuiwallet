...
test.describe('send btc', () => {
  ...
  test.describe('btc send form', () => {
    ...
    test('that fallbacks to other api provider if main fails', async ({ sendPage }) => {
      ...
      await sendPage.page.route(`${BESTINSLOT_API_BASE_URL_TESTNET}/inscription/in_transaction**`, async route => {
        return route.fulfill({
          status: 200,
      ...
      await sendPage.page.route(`${BESTINSLOT_API_BASE_URL_TESTNET}/inscription/single_info_id**`, async route => {
        return route.fulfill({
          status: 200,
          contentType: 'application/json',
          body: JSON.stringify({
