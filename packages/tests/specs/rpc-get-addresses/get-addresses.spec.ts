...
function softwareBeforeEach() {
  return () =>
    test.beforeEach(async ({ extensionId, onboardingPage }) => await onboardingPage.signInWithTestAccount(extensionId));
}

function ledgerBeforeEach(state: object) {
  return () =>
    test.beforeEach(
      async ({ extensionId, onboardingPage }) => await onboardingPage.signInWithLedgerAccount(extensionId, state),
    );
}
...
function getExpectedResponseForKeys(keys: SupportedBlockchains[]) {
  ...
  return {
    jsonrpc: '2.0',
    result: {
      addresses: [...(keys.includes('bitcoin') ? bitcoinKeys : []), ...(keys.includes('stacks') ? stacksKeys : [])],
    },
  };
}
...
async function initiateGetAddresses(page: Page, eventName: GetAddressesMethods) {
  return page.evaluate(async eventName => (window as any).LeatherProvider?.request(eventName), eventName);
}
...
getAddressesMethods.forEach(method => {
  test.describe(`Rpc: ${method}`, () => {
    test.beforeEach(async ({ extensionId, globalPage }) => await globalPage.setupAndUseApiCalls(extensionId));
    ...
    Object.entries(specs).forEach(([walletPreset, { beforeEach, expectedResult }]) => {
      test.describe(`${walletPreset} `, () => {
        ...
        if (walletPreset === 'softwareWallet') {
          test('it redirects back to get addresses flow when wallet is locked', async ({ homePage, page, context }) => {
            await homePage.lock();
            await page.goto('localhost:3000');
            const getAddressesPromise = initiateGetAddresses(page, method);
            const popup = await interceptRequestPopup(context);
          ...
          test('it returns the second accounts data after changing account', async ({ page, context }) => {
            await page.goto('localhost:3000');
            const getAddressesPromise = initiateGetAddresses(page, method);
            const popup = await interceptRequestPopup(context);
            ...
            await clickConnectLeatherButton(popup);
            const result = await getAddressesPromise;
            test.expect(result.result.addresses[0].address).toEqual('bc1qr9wmz342txkcmxxq5ysmfqrd5rvmxtu6nldjgp');
          });
        }
      });
    });
