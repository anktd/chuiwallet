...
test.describe('Send transfer (RPC)', () => {
  ...
  test('that the request can be broadcasted', async ({ page, context }) => {
    ...
    const [result] = await Promise.all([openSendTransfer(page)(baseParams), clickActionButton(context)('Approve')]);
  ...
  test('that the request can be cancelled', async ({ page, context }) => {
    const [result] = await Promise.all([openSendTransfer(page)(baseParams), clickActionButton(context)('Cancel')]);
