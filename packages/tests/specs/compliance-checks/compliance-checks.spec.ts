...
function mockChainalysisEntityRegistrationRequest(context: BrowserContext) {
  return async (routeHandler: (route: Route) => void) => {
    return context.route('https://api.chainalysis.com/api/risk/v2/entities', async route => routeHandler(route));
  };
}
...
test.describe('Compliance checks', () => {
  ...
  test('that it errors if non-compliant entity is detected', async ({ page, context }) => {
    ...
    await mockChainalysisEntityCheckRequest(context)(route => route.fulfill({ json: { risk: 'Severe' } }));

    const [leatherApprover] = await Promise.all([context.waitForEvent('page'), openIllegalTransfer(page)]);

    await test.expect(leatherApprover.getByText('Unable to handle request, errorCode: 1398')).toBeVisible();
  });
  ...
  test('nothing happens when chainalysis is down', async ({ page, context }) => {
    ...
    await test.expect(leatherApprover.locator('text="Unable to handle request, errorCode: 1398"')).toBeHidden();
