...
test.describe('Transaction signing', () => {
  ...
  test.describe('Contract calls', () => {
    test('that it validates against insufficient funds when performing a contract call', async ({ context }) => {
      ...
      const error = await transactionRequestPage.waitForTransactionRequestError('Insufficient balance');
