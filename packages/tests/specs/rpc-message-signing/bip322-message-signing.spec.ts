...
test.describe('Message signing', () =>
  test.describe('BIP-322 message signing', () => {
    ...
    test('Signature approved', async ({ page, context }) => {
      const [result] = await Promise.all([initiateMessageSigning(page)('test'), clickActionButton(context)('Sign')]);
    ...
    test('Signature rejected', async ({ page, context }) => {
      const [result] = await Promise.all([initiateMessageSigning(page)('test'), clickActionButton(context)('Cancel')]);
