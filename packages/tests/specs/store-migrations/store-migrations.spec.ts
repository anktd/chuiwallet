...
test.describe('Store migrations', () => {
  ...
  test.describe('Migration 0 --> 2', () => {
    ...
    test('that the app detects old store format', async ({ extensionId, globalPage }) => {
      ...
      await page.evaluate(
        async state =>
          new Promise(resolve => chrome.storage.local.set({ ['persist:root']: state }, () => resolve(true))),
      ...
      await page.evaluate(
        async () => new Promise(resolve => chrome.storage.local.onChanged.addListener(() => resolve(true))),
      );

      // State now in new format
      const result: RootState = await page.evaluate(
        async () =>
          new Promise(resolve => chrome.storage.local.get(['persist:root'], state => resolve(state['persist:root']))),
      );
      ...
      test.expect(result.softwareKeys.entities['default']?.salt).toEqual('c4cccf33166051f7704cd877a2f03f93');
