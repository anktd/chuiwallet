...
test.describe('Manage transaction', () => {
  ...
  test('that user can cancel fee', async ({ page }) => {
    ...
    const cancelActionSheet = page.getByTestId(`${ActivitySelectors.TransactionActionSheet}-cancel`);
  ...
  test('that user can increase fee', async ({ page }) => {
    ...
    const increaseFeeActionSheet = page.getByTestId(`${ActivitySelectors.TransactionActionSheet}-increase-fee`);

    const feeInput = increaseFeeActionSheet.getByTestId(ActivitySelectors.TransactionActionFeeInput);
    await feeInput.fill('0.004');
