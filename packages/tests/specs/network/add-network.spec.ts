...
test.describe('Networks tests', () => {
  test.beforeEach(async ({ extensionId, globalPage, onboardingPage, homePage, networkPage, page }) => {
  ...
  test('proper initial values on edit network', async ({ homePage, page, networkPage }) => {
    ...
    const stacksInputText = await networkPage.page.getByTestId(NetworkSelectors.NetworkStacksAddress).inputValue();
