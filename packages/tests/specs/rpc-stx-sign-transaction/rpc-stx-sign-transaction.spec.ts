...
import { MultiSigSpendingCondition, type TokenTransferPayloadWire, deserializeTransaction } from '@stacks/transactions';
import { TEST_ACCOUNT_1_PUBKEY, TEST_ACCOUNT_2_STX_ADDRESS, TEST_ACCOUNT_3_PUBKEY } from '@tests/mocks/constants';
...
test.describe('RPC: stx_signTransaction', () => {
  ...
  function initiateTxSigningSip30Format(page: Page) {
    return async (hex: string) =>
      page.evaluate(
        async transaction =>
          (window as any).LeatherProvider.request('stx_signTransaction', { transaction }).catch((e: unknown) => e),
        hex,
      );
  }
  ...
  test('that transaction details are the same after signing multi-signature STX transfer', async ({
    ...
    test
      .expect((deserializedUnsignedTxHex.auth.spendingCondition as MultiSigSpendingCondition).signaturesRequired)
      .toEqual((deserializedSignedTx.auth.spendingCondition as MultiSigSpendingCondition).signaturesRequired);
    test
    ...
    test.expect((deserializedSignedTx.auth.spendingCondition as MultiSigSpendingCondition).fields.length).toEqual(1);
  });
