...
import { type BtcSignerNetwork, makeNativeSegwitAddressIndexDerivationPath } from '@leather.io/bitcoin';
...
test.describe('Sign PSBT', () => {
  ...
  async function interceptBroadcastRequest(context: BrowserContext, callback: (route: Route) => Promise<void>) {
    const popup = await context.waitForEvent('page');
  ...
  test('that all inputs are signed even if the number of inputs is greater than vout index', async ({
    ...
    test.expect(result).toEqual(createExpectedResult(bytesToHex(psbt.toPSBT()), 'not-a-real-txid-response'));
  });
  ...
  test('that a failed request occurs if an invalid index is provided', async ({ page, context }) => {
    const psbt = createTestPsbt();
  ...
  test('that failed broadcasts return an error to the app', async ({ page, context }) => {
    const psbt = createTestPsbt();
    const requestPromise = interceptBroadcastRequest(context, route => route.fulfill({ status: 500 }));
