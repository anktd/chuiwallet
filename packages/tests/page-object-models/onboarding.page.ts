...
export class OnboardingPage {
  ...
  async signInWithTestAccount(id: string) {
    ...
    const isSignedIn = async () => {
      const { encryptionKey } = await this.page.evaluate(() => chrome.storage.session.get(['encryptionKey']));
  ...
  async signInWithLedgerAccount(id: string, state: object) {
    await this.page.evaluate(async walletState => chrome.storage.local.set({ 'persist:root': walletState }), state);
    await this.page.goto(`chrome-extension://${id}/index.html`);
  }
}
