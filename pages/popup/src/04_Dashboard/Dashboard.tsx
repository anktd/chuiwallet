...
export const Dashboard: React.FC = () => {
  ...
  const handleAccountClick = () => {
    setShowAccountSlide(true);
  };

  const handleCloseSlide = () => {
    setShowAccountSlide(false);
  };
  ...
  return (
    <div className="relative flex overflow-hidden flex-col items-center h-full bg-dark">
      ...
      <div className="flex gap-2.5 justify-between items-center mt-[52px] w-full text-lg font-medium leading-none text-center whitespace-nowrap max-w-[346px] text-foreground">
        <CryptoButton icon="popup/receive_icon.svg" label="Receive" onClick={() => navigate('/receive')} />
        <CryptoButton icon="popup/send_icon.svg" label="Send" onClick={() => navigate('/send')} />
      </div>
      ...
      <div className="flex flex-col w-full max-w-[346px] gap-[7px]">
        ...
        <CryptoBalance
          ...
          cryptoAmount="0 BCH"
          usdAmount="0 USD"
          icon="popup/bitcoin_cash_logo.svg"
        />
        <CryptoBalance cryptoName="USDT" cryptoAmount="0 USDT" usdAmount="0 USD" icon="popup/tether_usd_logo.svg" />
      </div>
        ...
        {showAccountSlide && (
          <motion.div
            ...
            <motion.div
              ...
              <button
                className="mt-6 bg-yellow-500 text-black rounded p-2 w-full font-bold"
                onClick={() => {
                  console.log('Generate account clicked');
                  // Add your generate account logic here
                }}>
                Generate account
              </button>
            </motion.div>
