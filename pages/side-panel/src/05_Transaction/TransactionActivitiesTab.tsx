...
const transactionActivitiesData: { date?: string; transactions: TransactionActivityItem[] }[] = [
  {
    date: 'Today',
    transactions: [
      {
...
export const TransactionList: React.FC = () => {
  return (
    <div className="flex overflow-hidden flex-col px-4 pt-5 bg-dark]">
      <img
        loading="lazy"
      ...
      <div className="flex flex-col mt-7 w-full">
        {transactionActivitiesData.map((transactionActivity, activityIndex) => (
          <React.Fragment key={activityIndex}>
            {transactionActivity.date && (
              <div className="text-xs font-bold leading-loose text-right text-foreground">
                {transactionActivity.date}
              </div>
            )}
            {transactionActivity.transactions.map((transaction, index) => (
              <TransactionActivityItem key={index} {...transaction} />
            ))}
          </React.Fragment>
        ))}
      </div>
